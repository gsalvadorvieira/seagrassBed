...
c     ------------------------- --------------
c     call prolong(m,nx(m),ny(m),nz(m),p(loco(m)),p(loco(m-1)) )
c     performs coarse to fine grid interpolation or "prolongation"
c     m is the coarse grid level
      implicit logical (a-z)
      integer i,j,k,iw,js,kb,nxm,nym,nzm
      double precision fin(0:2*nxm+1,0:2*nym+1,0:2*nzm+1),
     &     cor(0:nxm+1,0:nym+1,0:nzm+1)
      double precision an(8),bn(8),cn(8),dn(8),en(8),fn(8),
     &     gn(8),hn(8),x,y,z
c
c
      x=0.25
...
c      write(34,*) fin
      ...
      double precision pn(8),x,y,z,sum
