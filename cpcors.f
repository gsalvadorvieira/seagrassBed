...
c     ---------------------------------------
c     call cpcors(m,nx(m),ny(m),nz(m),cp(loccp(m)))
c     Computes the coefficients in the elliptic operator by averaging
c     the coefficients from the finer mesh.  Since the grid spacing
c     is doubled the coefficients should be divided by 4, but instead
c     we will multiply the rhs by 4.
